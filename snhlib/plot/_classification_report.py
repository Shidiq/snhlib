...
def plot_classification_report(
    X_train, X_test, y_train, y_test, model, report=False, return_df=False
):
    """
    ...
    if report:
        ...
        plt.figure(figsize=(10.72, 8.205))
        plt.subplots_adjust(wspace=0.4)

        plt.subplot(121)
        labels = y_train.unique()
        df_train = pd.DataFrame(
            classification_report(y_train, model.predict(X_train), labels=labels, output_dict=True)
        )
        ...
        df_test = pd.DataFrame(
            classification_report(y_test, model.predict(X_test), labels=labels, output_dict=True)
        )
